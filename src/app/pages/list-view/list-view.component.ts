...
export class ListViewComponent implements OnInit {
  ...
  ngOnInit() {
    this.route.queryParams.subscribe(params => {
      this.genreNames = params['genrenames']
        ? params['genrenames'].split(',').map((g: string) => this.capitalize(g.trim()))
        : [];
    ...
    this.route.queryParams.subscribe((params) => {
      ...
      this.currentPage = params['offset']
        ? parseInt(params['offset']) / this.itemsPerPage + 1
        : 1;
      this.pagesToShow = [];
  ...
  async loadFilteredMangas(
    ...
    const limit = limitP;
    const genres = genresP;
    const status = statusP;
    const category = categoryP;
    this.isLoading = true; // Start loading
    this.mangaService
      .getMangaList(offset, limit, genres, status, category)
